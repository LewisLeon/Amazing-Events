const UpcomingContent = document.getElementById("main")
...
/* creador de cartas principal */function Cartas(events) {
    let cont = ""
...
Cartas(eventos.EventUpcoming)
filtroMain(eventos.EventUpcoming)
//console.log(eventos.EventPast);
//console.log(eventos.EventUpcoming);
}

/* filtro de cartas en el UpcomingEvents */function filtroMain(events) {
    input.addEventListener('input', filtrador)
    checks.addEventListener('change',filtrador)
    Tarjetas(events)
    Checkboxes(events)
    function filtrador(){
        let firstFilter = textFilter(events,input.value)
